...
describe("Privacy and local-data reset", () => {
  ...
  it("confirms watchlist clearing with safe focus and preserves analytics", () => {
    ...
    fireEvent.click(clearButton);
    dialog = screen.getByRole("alertdialog", { name: "Clear saved titles?" });
  ...
  it("keeps the analytics-only reset narrow", () => {
    storeSavedTitle();
    window.localStorage.setItem(
      ANALYTICS_CONSENT_STORAGE_KEY,
  ...
  it("reports analytics reset failures without claiming persistent removal", () => {
    window.localStorage.setItem(
      ANALYTICS_CONSENT_STORAGE_KEY,
      '{"version":1,"choice":"declined"}',
    );
  ...
  it("reports a partial complete reset without claiming full persistent deletion", () => {
    ...
    expect(status).toHaveTextContent(
      "Current-session PickTonight data was reset, but browser storage could not be fully updated. The analytics choice may return after reload.",
    );
    expect(status).toHaveFocus();
  });
